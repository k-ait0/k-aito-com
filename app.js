...
const entries = [
  {id:"site-launch-trouble",title:"サイトを公開しようとしたら、早速つまずいた。",state:"MEMO",shelf:"technology",date:"2026.09.11",image:"notebook",tags:["サイト制作","XServer Static","GitHub"],summary:"独自ドメインへの切り替えで止まった、最初の公開トラブルの記録。",body:[
    ...
    ["p","今回はGitHubに `.htaccess` を作り、次のコメントを入れた。"],
    ["code","# XServer Static configuration"],
...
document.getElementById("projects-grid").innerHTML=`<a class="project-card" href="/projects/digital-storage/" aria-label="K. Aito DIGITAL STORAGEの詳細を読む"><span class="project-title">K. Aito / DIGITAL STORAGE</span><span class="project-image photo photo-notebook" role="img" aria-label="ノートとペンのイメージ"><span class="project-status"><small>ON GOING</small><span>公開中</span></span></span><span class="project-tags"><span>#Web</span><span>#制作記録</span></span><span class="project-description">このサイト自体の目的、構成、公開までの記録をまとめています。</span><span class="project-end"><span>PROJECT NOTE</span><span aria-hidden="true">→</span></span></a>`;

let lastRandom=[];
function renderRandom(){
  const pool=[...entries];
  for(let i=pool.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]];}
  const selected=pool.slice(0,Math.min(4,pool.length));
...
if(entries.length<=1)shuffleButton.hidden=true;
...
shelfSelect.addEventListener("change",renderArchive);
stateSelect.addEventListener("change",renderArchive);
document.getElementById("reset-filters").addEventListener("click",()=>{
  archiveSearch.value='';shelfSelect.value='all';stateSelect.value='all';projectsOnly=false;
  document.getElementById("archive-title").textContent="物置の中を探す";renderArchive();archiveSearch.focus();
});
...
if('IntersectionObserver' in window){
  ...
  document.querySelectorAll('#home,#shelves,#projects,#about').forEach(section=>navObserver.observe(section));
}
